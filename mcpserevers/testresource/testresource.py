...
def get_user_by_id(ctx: Context, user_id: int) -> str:
    """Return user details for a given user_id."""
    user = next((u for u in SAMPLE_DATA["users"] if u["id"] == int(user_id)), None)
    if user:
        return json.dumps(user, indent=2)
    return json.dumps({"error": "User not found"}, indent=2)

@mcp.resource("resource://products/{product_name}")
def get_product_by_name(ctx: Context, product_name: str) -> str:
    """Return product details for a given product name."""
    product = next((p for p in SAMPLE_DATA["products"] if p["name"].lower() == product_name.lower()), None)
    if product:
        return json.dumps(product, indent=2)
    return json.dumps({"error": "Product not found"}, indent=2)

@mcp.prompt()
async def print_product_details(product_name: str, ctx: Context) -> str:
    """Prints details of the specified product from the products resource in a table format using a resource template."""
    try:
        # Use the resource template to fetch the product
        product_data = await ctx.read_resource(f"resource://products/{product_name}")
        product = json.loads(product_data[0].content)
        # If the resource returns an error dict
        if isinstance(product, dict) and product.get("error"):
            return f"Product '{product_name}' not found."
        table = "| ID | Name | Price | Category |\n"
        table += "|----|------|-------|----------|\n"
        table += f"| {product['id']} | {product['name']} | ${product['price']:.2f} | {product['category']} |\n"
        return table
    except Exception as e:
        await ctx.error(f"Error printing product details: {str(e)}")
        raise

@mcp.prompt()
async def product_revenue_prompt(product_name: str, ctx: Context) -> str:
    """Get all users who brought this product and print order details.
    
    Args:
        product_name: Product name to get revenue details
        ctx: Context object for resource access
        
    Returns:
        str: Table formatted string containing the user details and total cost
    """
    try:
